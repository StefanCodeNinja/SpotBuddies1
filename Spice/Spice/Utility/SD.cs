using SpotBuddies.Models;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace SpotBuddies.Utility
{
    public static class SD
    {
        public const string DefaultFoodImage = "default_food.png";
        public const string ManagerUser = "Manager";
        public const string KitchenUser = "Kitchen";
        public const string FrontDescUser = "Frontdesk";
        public const string CustomerEndUser = "Customer";

        public const string ssShopingCartCount = "ssCartCount";
        public const string ssCouponCode = "ssCouponCode";

        //Statuses for order 
        public const string StatusSubmitted = "Submitted";
        public const string StatusInProcess = "Being Prepared";
        public const string StatusReady = "Ready for Dispatch";
        public const string StatusCompleted = "Completed";
        public const string StatusCencelled = "Canselled";
        //For payment
        public const string PaymentStatusPending = "Pending";
        public const string PaymentStatusApproved = "Approved";
        public const string PaymentStatusRejected = "Rejected";


        public static string ConvertToRawHtml(string source)
        {


            try
            {
                if (source.Length > 0)
                {

                    char[] array = new char[source.Length];
                    int arrayIndex = 0;
                    bool inside = false;

                    for (int i = 0; i < array.Length; i++)
                    {
                        char let = source[i];
                        if (let == '<')
                        {
                            inside = true;
                            continue;
                        }
                        if (let == '>')
                        {
                            inside = false;
                            continue;
                        }
                        if (!inside)
                        {
                            array[arrayIndex] = let;
                            arrayIndex++;
                        }
                    }
                    return new string(array, 0, arrayIndex);
                }
            }
            catch (NullReferenceException )
            {
                return new string("...");
            }
            return "...";
        }

        public static double DiscountedPrice(Coupon couponFromDb, double OriginalOrderTotal)
        {
            if (couponFromDb == null)
            {
                return Math.Round(OriginalOrderTotal, 2);
            }
            else
            {
                if (couponFromDb.MinimumAmount > OriginalOrderTotal)
                {
                    return Math.Round(OriginalOrderTotal, 2);
                }
                else
                {
                    //everithing is valid 
                    if (Convert.ToInt32(couponFromDb.CouponType) == (int)Coupon.ECouponType.Dollar)
                    {
                        //$10 off $100
                        return Math.Round(OriginalOrderTotal - couponFromDb.Discount, 2);
                    }
                    else
                    {
                        if (Convert.ToInt32(couponFromDb.CouponType) == (int)Coupon.ECouponType.Percent)
                        {
                            //%10 off $100
                            return Math.Round(OriginalOrderTotal - (OriginalOrderTotal * couponFromDb.Discount / 100), 2);
                        }
                    }
                }
                return Math.Round(OriginalOrderTotal, 2);
            }
        }
    }
}
